-- Rifa.GG — BANCO NOVO E VAZIO — 18/09/2026
-- Inclui cupons com mínimo de números e correção do sorteio.
-- Compatível com a versão que inclui Termos de Uso e logo do lobo.
-- Crie um projeto novo no Supabase e execute este arquivo inteiro no SQL Editor.
-- Não precisa executar o SQL de correção do sorteio separadamente.
-- Não contém contas, rifas, compras, cupons ou fotos de instalações anteriores.

-- Rifa.GG - banco completo para uma instalação nova e vazia.
-- Para atualizar um banco que já contém dados, use latest-update.sql.
...
create table if not exists public.raffles (
  ...
  total_numbers integer not null check (total_numbers between 10 and 500),
  ...
  winner_name text,
  winner_game_id text,
  winner_phone text check (winner_phone is null or winner_phone ~ '^[0-9]{1,6}$'),
...
create table if not exists public.accounts (
  ...
  login_failed_attempts integer not null default 0 check (login_failed_attempts >= 0),
  login_locked_until timestamptz,
  session_version integer not null default 1 check (session_version > 0),
  player_id text,
  phone text check (phone is null or phone ~ '^[0-9]{1,6}$'),
  role text not null default 'member' check (role in ('owner', 'admin', 'sponsor', 'member')),
...
create table if not exists public.purchases (
  ...
  buyer_game_id text not null,
  buyer_phone text not null check (buyer_phone ~ '^[0-9]{1,6}$'),
  numbers integer[] not null,
  subtotal_value bigint not null check (subtotal_value >= 0),
  total_value bigint not null check (total_value >= 0 and total_value <= subtotal_value),
  ...
  reviewed_by text,
  reviewed_by_id uuid references public.accounts(id) on delete set null
);
...
create table if not exists public.raffle_numbers (
  ...
  buyer_name text,
  buyer_game_id text,
  buyer_phone text check (buyer_phone is null or buyer_phone ~ '^[0-9]{1,6}$'),
...
begin
  ...
  values (trim(p_title), trim(p_description), p_price, p_total_numbers, p_drawing_date, nullif(trim(p_image_url), ''), 'active') returning id into v_raffle_id;
...
alter table public.coupons add column if not exists min_numbers integer not null default 1 check (min_numbers between 1 and 500);

create or replace function public.update_coupon_details(p_coupon_id uuid, p_discount_percent integer, p_min_numbers integer, p_actor_id uuid, p_actor text)
returns text language plpgsql security definer set search_path = public as $$
declare v_old coupons%rowtype;
begin
  if not exists (select 1 from accounts where id = p_actor_id and role in ('owner', 'admin', 'sponsor') and active = true) then raise exception 'Responsável inválido'; end if;
  ...
  if p_min_numbers is null or p_min_numbers not between 1 and 500 then raise exception 'O mínimo deve ser de 1 a 500 números'; end if;
  select * into v_old from coupons where id = p_coupon_id for update;
  if v_old.id is null then raise exception 'Cupom não encontrado'; end if;
  update coupons set discount_percent = p_discount_percent, min_numbers = p_min_numbers, updated_at = now() where id = p_coupon_id;
  insert into audit_logs(event, actor, details) values ('coupon_updated', p_actor, jsonb_build_object('actor_id', p_actor_id, 'coupon_id', p_coupon_id, 'old_discount', v_old.discount_percent, 'discount', p_discount_percent, 'old_min_numbers', v_old.min_numbers, 'min_numbers', p_min_numbers));
  return v_old.code;
end; $$;
revoke all on function public.update_coupon_details(uuid, integer, integer, uuid, text) from public, anon, authenticated;
grant execute on function public.update_coupon_details(uuid, integer, integer, uuid, text) to service_role;
...
begin
  ...
  if p_number_count < v_coupon.min_numbers then raise exception 'Este cupom exige pelo menos % números. Selecione mais % para usar.', v_coupon.min_numbers, v_coupon.min_numbers - p_number_count; end if;
  ...
  return jsonb_build_object('minNumbers', v_coupon.min_numbers, 'code', v_coupon.code, 'discountPercent', v_coupon.discount_percent, 'subtotal', v_subtotal, 'discountValue', v_discount, 'total', v_total);
end; $$;

drop function if exists public.create_coupon(text, integer, bigint, integer, timestamptz, uuid, text);
create or replace function public.create_coupon(
  ...
  p_expires_at timestamptz, p_actor_id uuid, p_actor text, p_min_numbers integer default 1
...
begin
  ...
  if p_min_numbers is null or p_min_numbers not between 1 and 500 then raise exception 'O mínimo deve ser de 1 a 500 números'; end if;
  insert into coupons (min_numbers, code, discount_percent, raffle_id, max_uses, expires_at, created_by, created_by_name)
  values (p_min_numbers, v_code, p_discount_percent, p_raffle_id, p_max_uses, p_expires_at, p_actor_id, trim(p_actor)) returning id into v_id;
...
begin
  ...
  update purchases set status = v_status, reviewed_at = now(), reviewed_by = trim(p_actor), reviewed_by_id = p_actor_id where id = p_purchase_id and status = 'pending';
  if not found then raise exception 'Pedido não encontrado ou já analisado'; end if;
  if p_action = 'approve' then
...
begin
  ...
  set status = 'available', buyer_name = null, buyer_game_id = null, buyer_phone = null, purchase_id = null, updated_at = now()
  where raffle_id = p_raffle_id;
  update raffles
  set status = 'active', winner_number = null, winner_name = null, winner_game_id = null, winner_phone = null
  where id = p_raffle_id;

  insert into audit_logs (event, actor, details)
  values ('raffle_reset', trim(p_actor), jsonb_build_object('raffle_id', p_raffle_id, 'title', v_title, 'actor_id', p_actor_id));
end; $$;
...
begin
  ...
  select title, status, price, original_price
  into v_old_title, v_status, v_old_price, v_original_price
  ...
  update raffles
  set title = trim(p_title), price = p_price, original_price = v_new_original_price,
      drawing_date = p_drawing_date, image_url = v_image_url
  where id = p_raffle_id;

  insert into audit_logs (event, actor, details)
  values ('raffle_updated', trim(p_actor), jsonb_build_object(
    'raffle_id', p_raffle_id, 'old_title', v_old_title, 'new_title', trim(p_title),
    'old_price', v_old_price, 'new_price', p_price, 'actor_id', p_actor_id
  ));

  return jsonb_build_object(
    'title', trim(p_title), 'price', p_price, 'originalPrice', v_new_original_price,
    'drawingDate', p_drawing_date, 'imageUrl', v_image_url
  );
end; $$;
...
begin
  ...
  insert into audit_logs (event, actor, details)
  values ('raffle_deleted', trim(p_actor), jsonb_build_object('raffle_id', p_raffle_id, 'title', v_title, 'actor_id', p_actor_id));
end; $$;
...
revoke execute on function public.create_coupon(text, integer, bigint, integer, timestamptz, uuid, text, integer) from public, anon, authenticated;
grant execute on function public.create_coupon(text, integer, bigint, integer, timestamptz, uuid, text, integer) to service_role;
...
-- Dados pessoais são acessados somente pelo servidor autenticado do site.
revoke all on function public.sync_coupon_used_count() from public, anon, authenticated;
revoke all on table public.accounts, public.raffles, public.purchases, public.raffle_numbers, public.coupons, public.audit_logs from public, anon, authenticated;
grant all on table public.accounts, public.raffles, public.purchases, public.raffle_numbers, public.coupons, public.audit_logs to service_role;
grant usage, select on sequence public.raffles_id_seq, public.raffle_numbers_id_seq, public.audit_logs_id_seq to service_role;

insert into storage.buckets (id, name, public)
values ('raffle-images', 'raffle-images', true)
on conflict (id) do update set public = true;

-- Correção do caminho da extensão pgcrypto, dentro da mesma transação.
do $correcao$
declare
  v_schema text;
  v_bytes integer;
begin
  ...
  if v_schema is null then
    raise exception 'A extensão pgcrypto não foi encontrada. Nenhuma correção foi aplicada.';
  end if;

  if pg_catalog.to_regprocedure('public.draw_raffle(bigint,uuid,text)') is null then
    raise exception 'A função draw_raffle desta versão não foi encontrada. Nenhuma correção foi aplicada.';
  end if;

  -- Verifica a função aleatória sem sortear nenhuma rifa.
  execute pg_catalog.format('select pg_catalog.octet_length(%I.gen_random_bytes(16))', v_schema)
    into v_bytes;
  if v_bytes is distinct from 16 then
    raise exception 'Falha ao validar pgcrypto. Nenhuma correção foi aplicada.';
  end if;

  -- Encontra pgcrypto tanto em public quanto em extensions (ou outro schema).
  -- Mantém pg_catalog primeiro e tabelas temporárias por último.
  execute pg_catalog.format(
    'alter function public.draw_raffle(bigint, uuid, text) set search_path = pg_catalog, public, %I, pg_temp',
    v_schema
  );
end;
$correcao$;
